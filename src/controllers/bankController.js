...
exports.transferFunds = async (req, res, next) => {
  try {
    // Only teller and manager can transfer funds
    if (!checkEmployeeRole(req.employee, ['teller', 'manager', 'admin'])) {
    ...
    // Basic suspicious activity detection (async)
    try {
...
exports.depositFunds = async (req, res, next) => {
  try {
    // Only teller and manager can deposit funds
    if (!checkEmployeeRole(req.employee, ['teller', 'manager', 'admin'])) {
...
exports.withdrawFunds = async (req, res, next) => {
  try {
    // Only teller and manager can withdraw funds
    if (!checkEmployeeRole(req.employee, ['teller', 'manager', 'admin'])) {
...
// Cancel a transaction (undo) — allows requester to cancel recent approved transactions
exports.cancelTransaction = async (req, res, next) => {
  try {
    ...
    if (transaction.status === 'pending' || transaction.status === 'rejected' || transaction.status === 'failed') {
      // pending can be removed by requester
      if (!transaction.requestedBy.equals(req.employee._id) && !checkEmployeeRole(req.employee, ['manager', 'admin'])) {
    ...
    // Allow quick undo for recently approved transactions by the original requester
    if (transaction.status === 'success') {
      ...
      const isRequester = transaction.requestedBy && transaction.requestedBy.equals(req.employee._id);
      if (!isRequester) {
